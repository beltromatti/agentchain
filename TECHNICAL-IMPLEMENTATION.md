...
This document describes how **AgentChain Engine v1.1.7**, the reference C client, realises the protocol specified in `PROTOCOL.md`. Read the protocol first; this document is a companion. Where the implementation deviates from the spec, the deviation is called out explicitly under `§ 9 Spec Deviations`.
...
The following table is the authoritative cross-reference. If a feature is in the protocol but not in this table, it is unimplemented in v1.1.7 (see `§ 9`).
...
### 9.6 Patch-release history (v1.0.1 → v1.1.7)
...
18. **v1.1.7** — monotonic vote convergence (fixes a **deadlock** the v1.1.6 lock introduced). The v1.1.6 lock was correct for safety but too rigid for liveness: under latency two validators can cast their first vote for a height in different slots over *different* proposal sets, latching onto two different blocks — and with a hard, never-released lock neither will switch, so neither block ever reaches the >½ floor and the chain wedges. This is exactly what happened immediately after the v1.1.6 rolling restart: seed and Frankfurt locked onto different empty blocks for h=336036 and stalled. Fix: replace the rigid lock with monotonic convergence. `vote_phase` now selects the lowest-**priority** proposal at the height it is trying to commit (tip+1) across *all* slots, and moves its vote only toward a *strictly lower* priority (re-affirming the current pick otherwise). Priority is a deterministic total order, so every validator walks down to the same global minimum: that block is the only one that can collect a quorum (≤ one finalises per height — no fork), and because everyone converges to it, it always gathers every honest vote (no deadlock). The happy path is unchanged — a height usually has one obvious minimum and commits in its first slot (3-node smoke + dropout/rejoin both pass). Note the residual property: a validator that switches toward a lower-priority block has signed two blocks at one height, which is technically equivocation; in this single-round design that is *benign* (only the global-minimum block can finalise, and v1 does not auto-generate `SLASH_EVIDENCE` — see `§ 9.5`). A two-phase (prevote/precommit) lock-and-unlock that converges without equivocation is the proper long-term design and is future work. Consensus-critical; all validators must run ≥ v1.1.7 (v1.1.6 is superseded and must not be left running — it deadlocks).
